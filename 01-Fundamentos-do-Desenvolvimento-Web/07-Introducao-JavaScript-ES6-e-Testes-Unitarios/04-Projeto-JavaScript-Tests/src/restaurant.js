...
const checkItem = (objItem, item, obj) => {
  let total = 0;
...
const checkItens = (obj) => {
  let total = 0;
  for (let index = 0; index < obj.consumption.length; index += 1) {
    total += checkItem(obj.consumption[index], obj.fetchMenu().drink, obj);
    total += checkItem(obj.consumption[index], obj.fetchMenu().food, obj);
  }
  return total;
};

const sumTotal = (obj) => {
  const total = checkItens(obj);
  const tax = total * 0.1;
  return (total + tax).toFixed(2);
};

const orderFromMenu = (obj) => {
  const createOrder = {
    order: (request) => {
      obj.consumption.push(request);
    },
  };
  Object.assign(obj, createOrder);
};
